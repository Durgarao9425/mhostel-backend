...
export const getFeePayments = async (req, res) => {
    ...
    try {
        ...
        let query = db('fee_payments as fp')
            ...
            .select('fp.payment_id', 'fp.fee_id', 'fp.student_id', 'fp.hostel_id', 'fp.amount as amount_paid', 'fp.payment_date', 'fp.transaction_id as transaction_reference', 'fp.receipt_number', 'fp.notes as remarks', 'fp.created_at', 'mf.fee_month as payment_for_month', 's.first_name', 's.last_name', 's.phone', 'h.hostel_name', 'pm.payment_mode_name as payment_mode');
...
export const getStudentPaymentHistory = async (req, res) => {
    try {
        ...
        let query = db('fee_payments as fp')
            .leftJoin('payment_modes as pm', 'fp.payment_mode_id', 'pm.payment_mode_id')
            .leftJoin('monthly_fees as mf', 'fp.fee_id', 'mf.fee_id')
            .select('fp.payment_id', 'fp.student_id', 'fp.payment_date', 'fp.amount as amount_paid', 'mf.fee_month as payment_for_month', 'fp.receipt_number', 'fp.transaction_id as transaction_reference', 'fp.notes as remarks', 'pm.payment_mode_name as payment_mode')
...
export const recordPayment = async (req, res) => {
    try {
        const { student_id, hostel_id, amount_paid, payment_mode_id, due_date, payment_date, transaction_reference, remarks } = req.body;
...
export const getReceipt = async (req, res) => {
    try {
        ...
        const payment = await db('fee_payments as fp')
            ...
            .leftJoin('monthly_fees as mf', 'fp.fee_id', 'mf.fee_id')
            .select('fp.payment_id', 'fp.fee_id', 'fp.student_id', 'fp.hostel_id', 'fp.amount as amount_paid', 'fp.payment_date', 'fp.transaction_id as transaction_reference', 'fp.receipt_number', 'fp.notes as remarks', 'fp.created_at', 'mf.fee_month as payment_for_month', 's.first_name', 's.last_name', 's.phone', 's.email', 'h.hostel_name', 'h.address', 'h.city', 'h.contact_number as hostel_contact', 'pm.payment_mode_name as payment_mode', 'r.room_number')

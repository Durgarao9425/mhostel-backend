...
export const createHostel = async (req, res) => {
    try {
        ...
        const { hostel_name, address, city, state, pincode, hostel_type, total_floors, rooms_per_floor, owner_id, amenities, admission_fee } = req.body;
        // Validation
...
export const getAllHostels = async (req, res) => {
    try {
        let query = db('hostel_master as h')
            ...
            .select('h.hostel_id', 'h.hostel_name', 'h.address', 'h.city', 'h.state', 'h.pincode', 'h.hostel_type', 'h.total_floors', 'h.owner_id', 'h.amenities', 'h.admission_fee', 'u.full_name as owner_name', 'u.phone as contact_number', 'u.email as email', 'h.created_at')
            .where({ 'h.is_active': 1 });
        ...
        const hostelsWithParsedAmenities = hostels.map(hostel => {
            ...
            if (hostel.amenities) {
                try {
                    ...
                    amenitiesArray = hostel.amenities.split(',').map((a) => a.trim()).filter(Boolean);
                }
            }
            return {
                ...hostel,
...
export const updateHostel = async (req, res) => {
    try {
        ...
        const { hostel_name, address, city, state, pincode, hostel_type, total_floors, owner_id, amenities, admission_fee } = req.body;
...
//# sourceMappingURL=hostelController.js.map
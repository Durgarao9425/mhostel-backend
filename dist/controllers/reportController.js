...
export const getIncomeReport = async (req, res) => {
    try {
        ...
        let query = db('fee_payments as fp')
            ...
            .select(db.raw('DATE_FORMAT(fp.payment_date, "%Y-%m") as month'), 'pm.payment_mode_name', db.raw('SUM(fp.amount) as total_amount'), db.raw('COUNT(*) as payment_count'))
...
export const getExpenseReport = async (req, res) => {
    try {
        ...
        let query = db('expenses as e')
            ...
            .select(db.raw('DATE_FORMAT(e.expense_date, "%Y-%m") as month'), 'ec.category_name', db.raw('SUM(e.amount) as total_amount'), db.raw('COUNT(*) as expense_count'))
...
export const getOccupancyTrends = async (req, res) => {
    try {
        ...
        let query = db('hostel_master as h')
            ...
            .leftJoin('room_types as rt', 'r.room_type_id', 'rt.room_type_id')
            .select('h.hostel_id', 'h.hostel_name', db.raw(`
          COALESCE(SUM(
            CASE 
              WHEN rt.room_type_name REGEXP '^[0-9]+$' THEN CAST(rt.room_type_name AS UNSIGNED)
              WHEN LOWER(rt.room_type_name) LIKE '%single%' THEN 1
...
export const getPaymentCollectionReport = async (req, res) => {
    try {
        ...
        let modeQuery = db('fee_payments as fp')
            ...
            .select('pm.payment_mode_name', db.raw('SUM(fp.amount) as total'), db.raw('COUNT(*) as count'))

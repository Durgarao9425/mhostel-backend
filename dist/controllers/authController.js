...
export const authController = {
    ...
    async login(req, res) {
        try {
            ...
            const user = await db('users')
                .select('users.user_id', 'users.email', 'users.password_hash', 'users.full_name', 'users.phone', 'users.role_id', 'users.hostel_id', 'users.is_active', 'user_roles.role_name')
    ...
    async me(req, res) {
        try {
            ...
            const user = await db('users')
                .select('users.user_id', 'users.email', 'users.full_name', 'users.phone', 'users.role_id', 'users.hostel_id', 'user_roles.role_name as role')
                .join('user_roles', 'users.role_id', 'user_roles.role_id')
    ...
    async forgotPassword(req, res) {
        try {
            ...
            const resetToken = jwt.sign({ user_id: user.user_id, email: user.email }, process.env.JWT_SECRET || 'your-super-secret-jwt-key-change-in-production', { expiresIn: process.env.PASSWORD_RESET_EXPIRES_IN || '1h' });

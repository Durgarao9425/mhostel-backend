...
const convertToDateOnly = (dateValue) => {
    if (!dateValue)
        return null;
...
export const getStudents = async (req, res) => {
    try {
        ...
        let query = db('students as s')
            ...
            .select('s.*', 'h.hostel_name', 'r.room_number', 'r.floor_number', 's.admission_date as check_in_date');
...
export const createStudent = async (req, res) => {
    try {
        const user = req.user;
        const { first_name, last_name, date_of_birth, gender, phone, email, guardian_name, guardian_phone, guardian_relation, permanent_address, present_working_address, id_proof_type, id_proof_number, id_proof_status, admission_date, admission_fee, admission_status, status, room_id, floor_number, monthly_rent } = req.body;
        ...
        if (finalStatusIsActive && updatedRent) {
            try {
                ...
                if (existingFee) {
                    ...
                    else if (parseFloat(existingFee.paid_amount || 0) > 0)

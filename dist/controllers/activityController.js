...
export const getRecentActivity = async (req, res) => {
    try {
        ...
        let paymentsQuery = db('fee_payments as fp')
            ...
            .select(db.raw("'payment' as type"), 'fp.payment_id as id', 'fp.payment_date as date', db.raw("CONCAT(s.first_name, ' ', s.last_name) as student_name"), 'fp.amount as amount', 'fp.created_at')
        ...
        let studentsQuery = db('students as s')
            ...
            .select(db.raw("'admission' as type"), 's.student_id as id', 's.admission_date as date', db.raw("CONCAT(s.first_name, ' ', s.last_name) as student_name"), 'r.room_number', 's.created_at')
            .where('s.status', 1)
        ...
        let expensesQuery = db('expenses as e')
            ...
            .select(db.raw("'expense' as type"), 'e.expense_id as id', 'e.expense_date as date', 'ec.category_name', 'e.amount', 'e.description', 'e.created_at')
        ...
        let incomeQuery = db('income as i')
            .select(db.raw("'income' as type"), 'i.income_id as id', 'i.income_date as date', 'i.source', 'i.amount', 'i.created_at')

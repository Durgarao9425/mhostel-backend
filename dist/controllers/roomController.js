...
const getCapacityFromRoomTypeName = (roomTypeName, description) => {
    if (roomTypeName) {
...
export const getRooms = async (req, res) => {
    try {
        ...
        let query = db('rooms as r')
            ...
            .select('r.*', 'rt.room_type_name as room_type_name', 'h.hostel_name');
        ...
        const roomsWithParsedAmenities = await Promise.all(rooms.map(async (room) => {
            ...
            if (room.amenities) {
                try {
                    ...
                    amenitiesArray = room.amenities.split(',').map((a) => a.trim()).filter(Boolean);
                }
            }
            ...
            const studentCount = await db('students')
                ...
                .count('* as count')
                .first();
            const occupiedCount = studentCount?.count ? parseInt(studentCount.count) : (room.occupied_beds || 0);
...
export const createRoom = async (req, res) => {
    try {
        const user = req.user;
        const { hostel_id, room_number, room_type_id, floor_number, occupied_beds, rent_per_bed, amenities } = req.body;
...
export const updateRoom = async (req, res) => {
    try {
        const { roomId } = req.params;
        const user = req.user;
        const { room_number, room_type_id, floor_number, occupied_beds, rent_per_bed, is_available, amenities } = req.body;
        ...
        if (room_type_id)
            updateData.room_type_id = room_type_id;
        if (floor_number !== undefined)
            updateData.floor_number = floor_number;
        if (occupied_beds !== undefined)
            updateData.occupied_beds = occupied_beds;
        if (rent_per_bed)
            updateData.rent_per_bed = rent_per_bed;

...
export const diagnoseCarryForward = async (req, res) => {
    console.log(`\n${'='.repeat(60)}`);
    ...
    try {
        ...
        res.json({
            ...
            calculation: prevMonthFee ? {
                ...
                total_due: parseFloat(prevMonthFee.total_due || 0),
                correct_carry_forward: Math.max(0, parseFloat(prevMonthFee.total_due || 0) - Math.max(parseFloat((await db('fee_payments').where('fee_id', prevMonthFee.fee_id).sum('amount as total'))[0]?.total || 0), parseFloat(prevMonthFee.paid_amount || 0)))
            } : null
        });
    }
    catch (error) {
...
async function cascadeUpdateFutureMonths(studentId, hostelId, paidMonth, queryBuilder) {
...
async function recalculateBalanceFromPayments(feeId) {
    try {
        const payments = await db('fee_payments')
            .where('fee_id', feeId)
            .sum('amount as total');
...
async function getPreviousMonthBalance(studentId, prevMonth, queryBuilder) {
    const qb = queryBuilder || db;
    try {
...
export const getMonthlyFees = async (req, res) => {
    try {
        ...
        let query = db('monthly_fees as mf')
            ...
            .select('mf.fee_id', 'mf.student_id', 'mf.hostel_id', 'mf.fee_month', 'mf.monthly_rent', 'mf.carry_forward', 'mf.total_due', 'mf.paid_amount', 'mf.balance', 'mf.fee_status', 'mf.due_date', 'mf.notes', 'mf.created_at', 'mf.updated_at', db.raw('COUNT(fp.payment_id) as payment_count'))
    ...
    try {
      ...
      `);
        ...
        let query = db('students as s')
            ...
            .select('s.student_id', 's.hostel_id', 's.first_name', 's.last_name', 's.phone', 's.email', 's.monthly_rent as student_monthly_rent', 'r.room_number', 's.floor_number', 's.admission_date', 's.status')
            .where('s.status', 1);
        ...
        if (monthlyFeesTableExists) {
            query = query.leftJoin('monthly_fees as mf', function () {
                ...
                .select('mf.fee_id', 'mf.fee_month', 'mf.monthly_rent as fee_monthly_rent', 'mf.carry_forward', 'mf.total_due', 'mf.paid_amount', 'mf.balance', 'mf.fee_status', 'mf.due_date');
        }
        else {
            // Add null values for fee columns if table doesn't exist
            query = query.select(db.raw('NULL as fee_id'), db.raw('? as fee_month', [currentMonth]), db.raw('NULL as fee_monthly_rent'), db.raw('0 as carry_forward'), db.raw('NULL as total_due'), db.raw('0 as paid_amount'), db.raw('NULL as balance'), db.raw('NULL as fee_status'), db.raw('NULL as due_date'));
        }
        // Authorization check
        if (user?.role_id === 2) {
            if (!user.hostel_id) {
        ...
        const studentsWithoutFee = results.filter((row) => row.fee_id === null);
...
export const getStudentAllPayments = async (req, res) => {
    try {
        ...
        const payments = await db('fee_payments as fp')
            .leftJoin('payment_modes as pm', 'fp.payment_mode_id', 'pm.payment_mode_id')
            .select('fp.payment_id', 'fp.fee_id', 'fp.student_id', 'fp.hostel_id', 'fp.amount', 'fp.payment_date', 'fp.due_date', 'fp.payment_mode_id', 'fp.transaction_id', 'fp.receipt_number', 'fp.notes', 'fp.created_at', 'fp.updated_at', 'pm.payment_mode_name as payment_method')
...
export const recordPayment = async (req, res) => {
    ...
    try {
        const { fee_id, student_id, hostel_id, amount, payment_date, due_date, payment_mode_id, transaction_id, receipt_number, notes } = req.body;
...
export const getPreviousMonthsFees = async (req, res) => {
    try {
        ...
        let query = db('monthly_fees as mf')
            .select('mf.fee_id', 'mf.fee_month', 'mf.monthly_rent', 'mf.carry_forward', 'mf.total_due', 'mf.paid_amount', 'mf.balance', 'mf.fee_status', 'mf.due_date')
            .where('mf.student_id', studentId);
        // Authorization check
        if (user?.role_id === 2) {
            if (!user.hostel_id) {
...
export const editCurrentMonthFee = async (req, res) => {
    try {
        const { feeId } = req.params;
...
export const recordAdjustment = async (req, res) => {
    ...
    try {
        const { feeId } = req.params;
        const { amount, transaction_type, reason, payment_date, payment_mode_id, notes } = req.body;
        const user = req.user;
...
export const recalculateFeeTotals = async (req, res) => {
    try {
        const { feeId } = req.params;
        const user = req.user;
        // Get the monthly fee record
...
export const recalculateCarryForwardForMonth = async (req, res) => {
...
export const updatePayment = async (req, res) => {
    try {
        ...
        const { amount, payment_date, due_date, payment_mode_id, receipt_number, transaction_id, notes } = req.body;
        // Validate payment ID
        ...
        const updatedPayment = await db('fee_payments')
            ...
            .select('fee_payments.payment_id', 'fee_payments.fee_id', 'fee_payments.student_id', 'fee_payments.hostel_id', 'fee_payments.amount', 'fee_payments.payment_date', 'fee_payments.due_date', 'fee_payments.payment_mode_id', 'fee_payments.transaction_id', 'fee_payments.receipt_number', 'fee_payments.notes', 'fee_payments.created_at', 'fee_payments.updated_at', 'payment_modes.payment_mode_name as payment_method')
            .where('payment_id', paymentIdNum)
            .first();
        res.json({
            success: true,
...
export const getCollections = async (req, res) => {
    try {
        ...
        let query = db('fee_payments as fp')
            ...
            .leftJoin('rooms as r', 's.room_id', 'r.room_id')
            .leftJoin('payment_modes as pm', 'fp.payment_mode_id', 'pm.payment_mode_id')
            .select('fp.payment_id', 'fp.amount', 'fp.payment_date', 'fp.receipt_number', 'fp.transaction_id', 'fp.notes', 'fp.created_at', db.raw("CONCAT(s.first_name, ' ', COALESCE(s.last_name, '')) as student_name"), 'r.room_number', 'pm.payment_mode_name as payment_mode')
...
//# sourceMappingURL=monthlyFeeController.js.map